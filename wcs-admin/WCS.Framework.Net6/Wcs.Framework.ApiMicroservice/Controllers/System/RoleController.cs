...
namespace Wcs.Framework.ApiMicroservice.Controllers
{
    ...
    public class RoleController : BaseSimpleRdController<RoleEntity>
    {
        ...
        [Permission("system:role:query")]
        [HttpGet]
        public async Task<Result> PageListForWcs([FromQuery] RoleEntity role, [FromQuery] PageParModel page)
        {
            return Result.Success().SetData(await _iRoleService.SelctPageListForWcs(role, page));
        }
