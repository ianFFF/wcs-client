...
namespace Wcs.Framework.ApiMicroservice.Controllers
{
    ...
    public class UserController : BaseSimpleRdController<UserEntity>
    {
        ...
        [HttpGet]
        [Permission("system:user:query")]
        public async Task<Result> PageListForWcs([FromQuery] UserEntity user, [FromQuery] PageParModel page, [FromQuery] long? deptId)
        {
            return Result.Success().SetData(await _iUserService.SelctPageListForWcs(user, page, deptId));
        }
