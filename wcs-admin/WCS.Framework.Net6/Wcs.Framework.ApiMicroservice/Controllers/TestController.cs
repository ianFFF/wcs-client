...
namespace Wcs.Framework.ApiMicroservice.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        private IWcsTestService _iWcsTestService;
        ...
        public TestController(IHubContext<MainHub> hub,
            ...
            IWcsTestService iWcsTestService,
            ...
            (_logger, _iWcsTestService, _iUserService, _iRoleService, _quartzInvoker, _hub, _local, _thumbnailSharpInvoer, _cacheDb) =

            (logger, iWcsTestService, iUserService, iRoleService, quartzInvoker, hub, local, thumbnailSharpInvoer, cacheInvoker);
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<Result> PlcJob()
        {
            WcstestEntity entity = new WcstestEntity();

            return Result.Success().SetData(await _iWcsTestService.AddEntity(entity));
        }
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<Result> PlcJobTest()
        {
            Dictionary<string, object> data = new Dictionary<string, object>()
            {
                {JobConst.method,"get" },
                {JobConst.url,"http://localhost:19001/api/Test/PlcJob" }
            };
            await _quartzInvoker.StartAsync("*/1 * * * * ?", "HttpJob", jobName: "plctest", jobGroup: "my", data: data);
        ...
        /// <returns></returns>
        [HttpPut]
        public async Task<Result> PlcStopJob()
        {
            await _quartzInvoker.StopAsync(new Quartz.JobKey("plctest", "my"));
            return Result.Success();
        }
