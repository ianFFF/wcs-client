...
namespace Wcs.Framework.WebCore.MiddlewareExtend
{
    ...
    {
        public static IServiceCollection AddSwaggerService<Program>(this IServiceCollection services, string title = "框架-API接口")
        {
            var apiInfo = new OpenApiInfo
            {
                Title = title,
                Version = "v1",
                Contact = new OpenApiContact { Name = "fanwd", Email = "fanweidi_815@163.com" }
            };
            #region 注册Swagger服务
        ...
        public static void UseSwaggerService(this IApplicationBuilder app, params SwaggerModel[] swaggerModels)
        {
            ...
            //    {
            //        c.SwaggerEndpoint("/v1/swagger.json", "Wcs.Framework");
            //    }
            //    else
            //    {
            ...
            app.UseSwaggerUI(c =>
            {
                if (swaggerModels.Length == 0)
                {
                    c.SwaggerEndpoint("/swagger/v1/swagger.json", "Wcs.Framework");
                }
                else
                {
                    foreach (var k in swaggerModels)
