<template>
  <div>
    <a-modal v-model="visible" :title="title" @ok="handleOk">
      ...
      <a-spin tip="Loading..." :spinning="loading">
        ...
        >
          ...
          <a-divider>第二步：选择目标货位</a-divider>
          <a-form-model-item label="货架" prop="shelves_id">
            <a-select v-model="form.shelves_id" placeholder="请选货架" :disabled="!form.equipment_id">
    ...
    <shelves-position ref="ShelvesPosition" :type="'rk'" @onFinish="onFinishSelectHw" />
  </div>
</template>
<script>
import { getAllEquipmentsManual } from '@/api/wcs/equipment'
import { selctListForManual } from '@/api/wcs/shelves'
import { manualRkMode } from '@/api/wcs/task'
...
export default {
  ...
  methods: {
    showModal() {
      ...
      this.getEquipments(this.ssxType)
    },
    handleOk(e) {
      if (e === 'cancel') {
        this.visible = false
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          this.loading = true
          manualRkMode({ startObject: this.form.equipment_id, endObject: this.form.shelves_position_id }).then(
            (res) => {
              this.loading = false
    ...
    getEquipments(type) {
      getAllEquipmentsManual({ type }).then((res) => {
        if (res.code === 200) {
